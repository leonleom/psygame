...
export class Results {
    ...
    render(gameData) {
        ...
        const cfiData = JSON.parse(localStorage.getItem('cfi_results'));
        const maiData = JSON.parse(localStorage.getItem('mai_results'));
        const demographicsData = JSON.parse(localStorage.getItem('demographics_results'));
        const rpmData = JSON.parse(localStorage.getItem('rpm_results'));
        ...
        if (cfiData) {
            this.displayContainer.appendChild(
                this.createQuestionnaireResultSection('cfi-report-section', '认知灵活性问卷 (CFI) 结果', cfiData.interpretation)
            );
            hasAnyData = true;
        } 
        if (maiData) {
            this.displayContainer.appendChild(
                this.createQuestionnaireResultSection('mai-report-section', '元认知意识问卷 (MAI) 结果', maiData.interpretation)
            );
            hasAnyData = true;
        }
        if (rpmData) {
            this.displayContainer.appendChild(
                this.createQuestionnaireResultSection('mai-report-section', '瑞文推理测验 结果', rpmData.interpretation)
            );
            hasAnyData = true;
        }
