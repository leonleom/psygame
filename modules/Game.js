/**
 * Game.js - 动态迷宫游戏模块 v31.0 (协议完全对齐版)
 * 1. 所有 logEvent 调用都严格遵循 "CA-BDAP v1.0" 协议。
 * 2. 增加了辅助函数来计算决策点和构建详细的事件负载。
 * 3. 将玩家输入和行动结果的事件日志分离开，以实现更精细的分析。
 */
export class Game {
    ...
    loadLevel() {
        ...
        if (this.level >= this.levels.length) { this.endGame(); return; }
        ...
        this.dataLogger.logEvent('level_start', {
            ...
            maze: {
                ...
                // 使用我们新计算出的、更准确的 optimalPath 长度
                optimalPathLength: optimalPath ? optimalPath.length : 0, // 如果找不到路径则为0
                decisionPoints: decisionPoints
            },
            settings: this.currentLevelConfig.settings,
            initialPatrols: this.currentLevelConfig.patrols.map(p => ({
                id: p.id,
                path: p.path
            }))
        });
    ...
    handlePlayerMove() {
        ...
        this.dataLogger.logEvent('player_intent_input', {
            ...
            gameStateAtIntent: {
                player: {
                    ...
                    steps: this.steps,
                    isStunned: this.isPlayerStunned
                }
            }
        });
        ...
        this.dataLogger.logEvent('player_action_result', {
            inputKey: key,
            result: result,
            from: fromPos,
            to: toPos
        ...
        for (let i = this.currentLevelConfig.dynamicEvents.length - 1; i >= 0; i--) {
            ...
            if (evt.triggerOnProgress) {
                ...
                if(currentItem === evt.triggerOnItem) {
                    ...
                    if (conditionMet) {
                        this.dataLogger.logEvent('item_interaction', {
                            itemType: 'key',
                            position: { ...this.player }
                        });
                        triggered = true;
    ...
    _logLevelEnd(status) {
        // 1. 【新增】立刻阻止后续的 update 逻辑运行
        if (this.isLevelEnding) return; // 如果已经在结束流程中，则直接返回
        this.isLevelEnding = true; // 设置一个标志位

        const duration = Date.now() - this.levelStartTime;
        
        // 2. 记录 level_end 事件 (逻辑不变)
        ...
        // 4. 【新增】使用 setTimeout 来异步加载下一关
        // 这会给当前帧一个结束的机会，并确保在进入下一关的加载流程前，
        // 不会再有任何 update 循环被执行。
        setTimeout(() => {
            this.loadLevel();
        }, 50); // 50毫秒的延迟足够了
    ...
    endGame() {
        this.stopGameLoop(); this.stopListening();
